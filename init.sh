...
create_sudo_user() {
    ...
    if id "$username" &>/dev/null; then
        ...
        read -p "是否重置该用户的配置？[y/N]: " reset_user
        if [[ ! $reset_user =~ ^[Yy]$ ]]; then
            return 1
        fi
    else
        # 创建新用户
    ...
    echo "用户 $username 创建完成，已添加到 sudo 组"
    return 0
}
...
setup_swap() {
    ...
    if swapon -s | grep -q "$swapfile"; then
        echo "已存在 swap 文件"
        read -p "是否重新创建？[y/N]: " recreate_swap
...
if [ -n "$NEW_USERNAME" ]; then
    print_success "已创建管理用户: $NEW_USERNAME"
    print_info "已禁用 root SSH 登录"
fi
